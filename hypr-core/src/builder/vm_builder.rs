...
mod tests {
    ...
    #[test]
    fn test_build_command_json_run() {
        let adapter = mockall::mock! {
            Adapter {}
            impl VmmAdapter for Adapter {
        ...
        assert!(json.contains("apk add nginx"));
        assert!(json.contains("/workspace"));
    }

    #[test]
    fn test_build_command_json_finalize() {
        let adapter = mockall::mock! {
            Adapter {}
            impl VmmAdapter for Adapter {
                async fn create(&self, config: &VmConfig) -> Result<VmHandle>;
        ...
        assert!(json.contains("layer-abc123"));
    }
}
